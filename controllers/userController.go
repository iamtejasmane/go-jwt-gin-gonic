...
import (
	...
	helper "github.com/iamtejasmane/go-jwt-gin-gonic/helpers"
	"github.com/iamtejasmane/go-jwt-gin-gonic/models"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if count > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "This email/phone number is already exists"})
		}
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		passwordIsValid, msg := VerfiyPassword(*user.Password, *foundUser.Password)
		defer cancel()
...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		groupStage := bson.D{{Key: "$group", Value: bson.D{
			{Key: "_id",
				Value: bson.D{{Key: "_id", Value: "null"}},
			},
			{Key: "total_count",
